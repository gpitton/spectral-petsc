...
int main(int argc,char **args)
{
  ...
  ac->dim[0] = 128; ac->dim[1] = 128; // default dimension extent
  //ac->dim[2] = 32; //ac->dim[3] = 32;

  //ac->dim[0] = 8; ac->dim[1] = 6; // default dimension extent
  ac->debug = 0; ac->exact = 0; ac->gamma = 1.e0; ac->exponent = 2.0;
  ...
  ierr = DMDACreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_PERIODIC,DM_BOUNDARY_PERIODIC,DMDA_STENCIL_STAR,ac->dim[0]-2,ac->dim[1]-2,PETSC_DECIDE,PETSC_DECIDE,1,1,NULL,NULL,&da);CHKERRQ(ierr);
  ierr = DMDASetUniformCoordinates(da,0.,2*PETSC_PI,0.,2*PETSC_PI,0.,0.);CHKERRQ(ierr);
  ...
  char sbuf[13];
#ifdef _GP_USE_HDF5_
  sprintf(sbuf,"output%d.h5",ac->d);
  ierr = PetscViewerHDF5Open(PETSC_COMM_WORLD,sbuf,FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
  //ierr = PetscViewerHDF5GetBaseDimension2(viewer,PETSC_TRUE);CHKERRQ(ierr);
  //PetscObjectViewFromOptions((PetscObject) viewer,NULL,"-my_obj_view");
#else
  //sprintf(sbuf,"output%d.vtk",ac->d);
  //PetscViewerASCIIOpen(PETSC_COMM_WORLD,sbuf,&viewer);
  //PetscViewerSetFormat(viewer,PETSC_VIEWER_ASCII_VTK);
  sprintf(sbuf,"output%d.vts",ac->d);
  PetscViewerVTKOpen(PETSC_COMM_WORLD,sbuf,FILE_MODE_WRITE,&viewer);
  //PetscViewerSetFormat(viewer,PETSC_VIEWER_DEFAULT);
#endif
  DMView(da,viewer);
  PetscObjectSetName((PetscObject) x, "x");
  VecView(x,viewer);
  PetscPrintf(PETSC_COMM_WORLD,"written output on %s\n",sbuf);
